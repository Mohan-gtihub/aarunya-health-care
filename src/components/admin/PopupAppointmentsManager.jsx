...
export default function PopupAppointmentsManager({ showMessage }) {
    ...
    useEffect(() => {
        ...
        const savedReadItems = localStorage.getItem('readPopupAppointments');
        if (savedReadItems) {
    ...
    const markAsRead = (id) => {
        ...
        localStorage.setItem('readPopupAppointments', JSON.stringify([...newReadItems]));
    };
    ...
    const unreadCount = popupRequests.filter(req => !readItems.has(req.id)).length;
            ...
            {popupRequests.length === 0 ? (
                ...
                <div className="requests-grid">
                    {popupRequests.map((request) => {
                        const isUnread = !readItems.has(request.id);
                        return (
                            <div
                                key={request.id}
                                className={`request-card ${isUnread ? 'unread' : 'read'}`}
